...
- (void) peripheral:(CBPeripheral *)_peripheral didDiscoverServices:(NSError *)error {
    ...
    for (CBService *service in services) {
        HBLog(@"Services %@ (%@)", [service UUID], service);
        if ([[service.UUID UUIDString] isEqualToString:@"FEF5"]||[service.UUID isEqual:[CBUUID UUIDWithString:SPOTA_REQUEST_IS_BIND_SERVICE_UUID]]) {
...
- (void) peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    HBLog(@"Characteristics for service %@ (%@)", [service UUID], service);
    NSArray *characteristics = [service characteristics];
    for (CBCharacteristic *characteristic in characteristics) {
        HBLog(@" -- Characteristic %@ (%@)", [characteristic UUID], characteristic);
        ...
        if ([characteristic.UUID.UUIDString isEqualToString:SPOTA_REQUEST_IS_BIND_UUID]) {
            [[NSNotificationCenter defaultCenter]postNotificationName:@"DiscoverBindCharacteristics" object:nil];

            return ;
        }
    }
    [[NSNotificationCenter defaultCenter]postNotificationName:@"DiscoverCharacteristics" object:nil];
    [self.delegate deviceReady:self.device];
}

- (void) peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    if (!error) {
        HBLog(@"Value for %@ is %@", characteristic.UUID, characteristic.value);
        [[NSNotificationCenter defaultCenter] postNotificationName:GenericServiceManagerDidReceiveValue object:characteristic];
    } else {
        HBLog(@"Failed to read characteristic %@", characteristic.UUID);
...
- (id) initWithCoder:(NSCoder *) decoder {
    if (self = [super init]) {
        HBLog(@"Name: %@", [decoder decodeObjectForKey:@"identifier"]);
...
- (void) writeValue:(CBUUID*)serviceUUID characteristicUUID:(CBUUID*)characteristicUUID p:(CBPeripheral *)p data:(NSData *)data andResponseType:(CBCharacteristicWriteType)responseType
{
    ...
    if (!service) {
        HBLog(@"Could not find service with UUID %s on peripheral with UUID %@\r\n",[self CBUUIDToString:serviceUUID], p.identifier);
        return;
    }
    CBCharacteristic *characteristic = [self findCharacteristicFromUUID:characteristicUUID service:service];
    if (!characteristic) {
        HBLog(@"Could not find characteristic with UUID %s on service with UUID %s on peripheral with UUID %@\r\n",[self CBUUIDToString:characteristicUUID],[self CBUUIDToString:serviceUUID], p.identifier);
        return;
    }
